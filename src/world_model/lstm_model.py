"""
PyTorch Multi-Task LSTM World Model Architecture for NEXUS-Forecast.
Processes historical states (batch_size, 10, 22) through a 2-layer unidirectional LSTM,
...
class LSTMWorldModel(nn.Module):
    def __init__(
        ...
        # Latent projection / LayerNorm for stability
        self.latent_norm = nn.LayerNorm(hidden_size)
        self.dropout = nn.Dropout(dropout)

        # 2. Multi-Task Prediction Heads per Forecast Horizon K
    ...
    def forward(self, x: torch.Tensor) -> Dict[str, Dict[int, torch.Tensor]]:
        ...
        final_h = lstm_out[:, -1, :]  # Shape: (batch_size, hidden_size)
